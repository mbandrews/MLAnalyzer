...
#parser.add_argument('-i', '--img_inputs', default='LISTS/Era2017_15Mar2021_CLUSv1_data2017-Run2017B.txt', type=str, help='List file of img inputs.')
parser.add_argument('-i', '--img_inputs', default='LISTS/Era2017_24Oct2021_CLUSv1_data2017-Run2017C.txt', type=str, help='List file of img inputs.')
...
def apply_3b3_mask(img, ieta0, iphi0, ieta1, iphi1):

    mask = np.zeros_like(img)
    ctr = 15
    mask[:, ctr-1:ctr+2,ctr-1:ctr+2] = 1.

    dieta = ieta1 - ieta0
    diphi = iphi1 - iphi0
    # NOTE: image window here should correspond to the one used in RHAnalyzer
    off = img.shape[-1]//2
    #print('diff_ieta:', dieta)
    #print('diff_iphi:', diphi)

    # ------------------------------------------------
    # iphi (col) wrap-around
    # ------------------------------------------------
    sat_ieta = int(ctr + dieta)
    if 360 - abs(diphi) < 15:
        #print('!! WRAPAROUND !!')
        if diphi > 0:
            # Wrap-around on left side
        ...
        sat_iphi = diphi
    sat_iphi = int(ctr + sat_iphi)
    #print('sat_iphi:', sat_iphi)
    mask[:, sat_ieta-1:sat_ieta+2, sat_iphi-1:sat_iphi+2] = 1.
...
for i,fi in enumerate(img_inputs):
    tree.Add(fi)
    #break
    #if i >= 1000: break
...
outdir = args.outdir
outdir = '/storage/local/data1/gpuscratch/mba2012/'+outdir
if not os.path.isdir(outdir):
    os.makedirs(outdir)
file_out = ROOT.TFile.Open("%s/%s_mantuple.root"%(outdir, sample), "RECREATE")
...
#'''
hltMass55 = np.zeros(1, dtype='float32')
tree_out.Branch('hltMass55', hltMass55, 'hltMass55/F')
hltMass90 = np.zeros(1, dtype='float32')
tree_out.Branch('hltMass90', hltMass90, 'hltMass90/F')
...
dR3b30 = np.zeros(1, dtype='float32')
dR3b31 = np.zeros(1, dtype='float32')
tree_out.Branch('dR3b30', dR3b30, 'dR3b30/F')
tree_out.Branch('dR3b31', dR3b31, 'dR3b31/F')
#'''
mamask0 = np.zeros(1, dtype='float32')
mamask1 = np.zeros(1, dtype='float32')
tree_out.Branch('mamask0', mamask0, 'mamask0/F')
tree_out.Branch('mamask1', mamask1, 'mamask1/F')

#eb_energy0 = np.zeros(32*32, dtype='float32')
#tree_out.Branch('eb_energy0', eb_energy0, 'eb_energy0[1024]/F')
#eb_energy3b30 = np.zeros(32*32, dtype='float32')
#tree_out.Branch('eb_energy3b30', eb_energy3b30, 'eb_energy3b30[1024]/F')
#
#eb_energy1 = np.zeros(32*32, dtype='float32')
#tree_out.Branch('eb_energy1', eb_energy1, 'eb_energy1[1024]/F')
#eb_energy3b31 = np.zeros(32*32, dtype='float32')
#tree_out.Branch('eb_energy3b31', eb_energy3b31, 'eb_energy3b31[1024]/F')
...
zero_mask = np.zeros((32,32))
zero_mask[12:19,11:20] = 1.
...
for iEvt in range(iEvtStart,iEvtEnd):
    ...
    for i in range(npho_roi):
        ...
        #print('pho:%d, ieta:%d'%(i, ieta[-1][0]))
        #print('pho:%d, iphi:%d'%(i, iphi[-1][0]))
    ...
    sc_cms = np.array(sc_cms) #*zero_mask
    ...
    #if tree.pho_chgIso[1]/tree.pho_pT[1] > 0.05: continue
    #'''
    hltMass55[0] = tree.hltMass55
    hltMass90[0] = tree.hltMass90
    ...
    # Forgot to include energyCorr in Era2017_25Jun2021_CLUSv1_data2017-Run2017B.txt, so need to set to default value
    energyCorr0[0] = -1 #if '2018' in sample else tree.pho_ecalEPostCorr[0]
    energyCorr1[0] = -1 #if '2018' in sample else tree.pho_ecalEPostCorr[1]
    ...
    dR3b30[0] = tree.pho_dR3b3[0]
    dR3b31[0] = tree.pho_dR3b3[1]

    for i in range(npho_roi):
        pass
        #print('pho:%d, seed:%d, ieta:%d'%(i, 0, tree.pho_ieta03b3[i]))
        #print('pho:%d, seed:%d, iphi:%d'%(i, 0, tree.pho_iphi03b3[i]))
        #print('pho:%d, seed:%d, ieta:%d'%(i, 1, tree.pho_ieta13b3[i]))
        #print('pho:%d, seed:%d, iphi:%d'%(i, 1, tree.pho_iphi13b3[i]))

    #print(sc_cms.shape)
    sc_cms_3b3 = np.array([apply_3b3_mask(sc_cms[i], ieta0=tree.pho_ieta03b3[i], iphi0=tree.pho_iphi03b3[i], ieta1=tree.pho_ieta13b3[i], iphi1=tree.pho_iphi13b3[i]) for i in range(npho_roi)])

    ma = inv_transform(resnet([torch.Tensor(sc_cms_3b3).cuda()/eb_scale,\
                               torch.Tensor(iphi).cuda()/360.,\
                               torch.Tensor(ieta).cuda()/170.
                              ])).tolist()
    ma0_, ma1_ = ma[0][0], ma[1][0]
    #print(ma0_, ma1_)
    mamask0[0] = ma0[0] if ma3b30[0] <  0. else ma0_
    mamask1[0] = ma1[0] if ma3b31[0] <  0. else ma1_

    ##pho0, Et
    #sc_cms_ = sc_cms[0,0].flatten()
    #sc_cms_3b3_ = sc_cms_3b3[0,0].flatten()
    #for i in range(1024):
    #    eb_energy0[i] = sc_cms_[i]
    #    eb_energy3b30[i] = sc_cms_3b3_[i]
    ##pho1, Et
    #sc_cms_ = sc_cms[1,0].flatten()
    #sc_cms_3b3_ = sc_cms_3b3[1,0].flatten()
    #for i in range(1024):
    #    eb_energy1[i] = sc_cms_[i]
    #    eb_energy3b31[i] = sc_cms_3b3_[i]
